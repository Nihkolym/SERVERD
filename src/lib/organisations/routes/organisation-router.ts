...
const uploadAvatar = multer({ storage: avatarStorage }).single("logo");
...
class UserRouter {
  ...
  private setRoutes() {
    this.router.get("/me", OrganisationController.getMyOrganisations);
    ...
    this.router.post(
      ...
      AuthController.signOrganisationUp,
    );
